...
\begin{contribution}C3 --- contribuição computacional. Integra linhagem, auditoria de vazamento, validação cruzada agrupada com ajuste estritamente no treino, controles negativos e 611 configurações de estabilidade para tornar a crítica verificável, e não apenas conceitual.\end{contribution}
...
\begingroup\SingleSpacing\fontsize{9.2}{10.7}\selectfont\setlength{\tabcolsep}{3.5pt}\renewcommand{\arraystretch}{1.2}\begin{longtable}[]{@{}
...
Validade interna e de conclusão. O \texttt{group\_id} bloqueia duplicidades prováveis no limiar 0,999. A revisão humana dos pares próximos ao limiar foi concluída sem erros identificados no material revisado, fortalecendo a adequação operacional dessa regra, mas sem garantir identidade única nem excluir duplicidades abaixo do limiar. Uma análise de sensibilidade futura pode repetir a construção em 0,995, 0,997, 0,999 e 0,9995 e recalcular as métricas por limiar. Também deve ablar a regra do alvo --- maior agrupamento, agrupamento aleatório, mais compacto e mais separado --- para quantificar a influência dessa convenção. Os limiares de relevância atuais permanecem heurísticos; distribuições contínuas sustentam a inferência principal.

Reprodutibilidade e limites remanescentes. Em consonância com boas práticas de pesquisa computacional reprodutível (Sandve et al., 2013), a edição definitiva do código e da documentação está congelada na tag limpa \texttt{paper-v1.1-final}, commit \texttt{6b81953}, eliminando a ambiguidade sobre qual estado do repositório corresponde à versão final do artigo. A proveniência da corrida científica anterior continua registrada como \texttt{worktree\_dirty=true} e não é reescrita retroativamente. Permanecem como limitações documentadas o fato de o SHA-256 de \path{w600k_r50.onnx} não ter sido registrado no manifesto da execução FairFace, a ausência de predições OOF por observação nos outputs preservados e a ausência de replicação independente. Ambiente congelado, hashes de pesos, manifesto completo, preservação de predições OOF e depósito estruturado com DOI devem integrar qualquer nova corrida destinada a ampliar ou replicar os resultados; esses itens não são apresentados como se tivessem sido reconstruídos retroativamente.
