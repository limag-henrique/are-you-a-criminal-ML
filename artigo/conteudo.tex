...
\begin{boxtext}RESULTADO EMPÍRICO 1, EM LINGUAGEM DIRETA --- ROC-AUC = 0,896 coexistiu com Jaccard mediano do alvo = 0,004. O sistema recuperou com alta precisão a regra endógena criada no treino, mas mudou radicalmente quem constituía o fenômeno aprendido quando a configuração foi perturbada. Desempenho fora da amostra não estabelece validade de construto quando o próprio alvo é gerado pela representação avaliada.\end{boxtext}

As perguntas de pesquisa são: RQ1 --- quem entra no corpus, quem fica de fora e por que uma lista de procurados não representa criminalidade populacional? RQ2 --- quão bem o alvo criado pelo próprio pipeline pode ser recuperado? RQ3 --- esse alvo permanece o mesmo quando mudam semente, k, ordem, \texttt{batch\_size} e representação? RQ4 --- o que os dados permitem ou impedem afirmar sobre criminalidade, raça e grupos humanos? RQ5 --- em um domínio externo com categorias demográficas documentadas, a composição da base está associada a mudanças no padrão que o algoritmo encontra mesmo quando as métricas agregadas parecem semelhantes?

\section{Contribuições}

\begin{contribution}C1 --- formalização e demonstração por construção. A Proposição~1 mostra que um alvo endógeno pode ser recuperado com AUC arbitrariamente próxima de 1 enquanto um critério externo independente permanece no acaso; o caso executável em \path{scripts/run_proposition_demo.py} reproduz a construção e deposita resultados estruturados em \path{artifacts/proposition/proposition_results.csv}.\end{contribution}
...
\begin{contribution}C3 --- contribuição computacional. Integra linhagem, auditoria de vazamento, validação cruzada agrupada com ajuste estritamente no treino, controles negativos e 611 configurações de estabilidade para tornar a crítica verificável, e não apenas conceitual. Inclui ainda scripts implementados e pré-declarados para ablação da definição do alvo (\path{run_target_ablation.py}), sensibilidade do \texttt{group\_id} (\path{run_groupid_sensitivity.py}) e replicação pareada do FairFace (\path{run_fairface_replicated.py}), cujas grades confirmatórias permanecem pendentes de execução integral.\end{contribution}
...
Uma extensão pareada e replicada foi implementada em \path{scripts/run_fairface_replicated.py}. Para cada replicação, ela sorteia um núcleo estratificado compartilhado entre A--D e complementos independentes por cenário, executa as combinações declaradas de semente e $k$, preserva predições OOF por registro em Parquet e decompõe AUC, Jaccard e tamanho do alvo entre cenário, replicação, semente, $k$ e resíduo. Os defaults pré-declarados são 50 replicações, cinco sementes por replicação e $k\in\{32,64,128\}$. O script está implementado e seu contrato de saída está definido, mas a grade confirmatória de 50 replicações ainda não foi executada integralmente; os resultados desta edição permanecem os do desenho original e nenhuma estimativa de smoke test é usada como evidência substantiva.

\subsection{Ablação pré-declarada da definição do alvo}

Seis regras públicas foram implementadas em \path{face_profile_ml/target_rules.py}: maior agrupamento, mais compacto, mais separado, aleatório, central e outlier. O executor \path{scripts/run_target_ablation.py} aplica cada regra sob a mesma divisão agrupada, semente e $k$, calcula AUC e PR-AUC OOF e compara os conjuntos-alvo por Jaccard. O contrato de saída inclui \path{artifacts/ablation/ablation_results.csv} e \path{artifacts/ablation/incompatibility_matrix.csv}. A grade confirmatória de 50 sementes está pré-declarada e os scripts estão implementados, mas a corrida integral não foi realizada; por isso, esta edição não reporta uma contagem de regras incompatíveis com AUC acima de 0,85 e não converte resultados reduzidos de verificação em achado científico.
...
No corpus principal, o mesmo padrão emerge de forma ainda mais saliente. ROC-AUC = 0,896 e Jaccard mediano do alvo = 0,004 coexistem na mesma análise: o sistema recupera com alta consistência a regra que o próprio pipeline produziu, enquanto a identidade de quem pertence ao alvo muda radicalmente de uma configuração para outra. A Proposição 1 estabelece que essa coexistência não é uma coincidência empírica, mas uma consequência estrutural de pipelines com alvo endógeno. A Figura 2 sintetiza esse paradoxo.

\begin{boxtext}\textbf{Resultado Empírico 1.} ROC-AUC = 0,896 (variação de 0,882 a 0,904 entre dobras) coexiste com Jaccard mediano do alvo = 0,004 (IQR 0,000--0,047) entre sementes em $k=64$. Um sistema pode exibir desempenho preditivo aparentemente excelente enquanto muda radicalmente quem constitui o fenômeno aprendido. Esse paradoxo não é uma falha pontual do protocolo: a Proposição 1 mostra que ele é estruturalmente inevitável quando label e escore compartilham a mesma geometria.\end{boxtext}
...
\sourcecaption{Fonte: elaboração própria a partir de \path{cross_fitted_metrics.csv} e \path{stability_summary.csv}. As métricas do painel A avaliam o alvo endógeno; os demais painéis resumem perturbações de semente e k.}
...
Validade de construto, externa e racial. O alvo é sintético e endógeno à geometria facial; não mede criminalidade, culpa, raça/cor, identidade ou equidade. O corpus decorre de seleção institucional, divulgação, permanência pública, disponibilidade de fotografia, coleta e processamento, sem amostragem probabilística. A percepção visual que motivou a questão racial permanece uma hipótese, não um resultado: sem raça/cor validada por registro, fonte recuperada e denominadores compatíveis, a monografia não pode estimar quantas pessoas negras, brancas ou de qualquer outro grupo aparecem nas listas nem calcular seletividade racial. Essa ausência não deve ser preenchida por inferência visual ou por classificação automática de raça.

\section{Validade externa}

Braço demográfico externo. Os rótulos do FairFace são categorias históricas do dataset, não autoidentificação irrestrita nem categorias biológicas. Como cada cenário altera proporções e registros, e a estabilidade entre sementes é baixa, não se estima efeito causal demográfico. Uma extensão adequada exige 50--100 replicações independentes ou um desenho pareado que maximize um núcleo comum, permitindo decompor composição, amostragem, k, semente e interações; o script \path{run_fairface_replicated.py} implementa esse protocolo, mas a grade confirmatória completa ainda não foi executada.

\section{Validade interna}

O \texttt{group\_id} bloqueia duplicidades prováveis no limiar 0,999. A revisão humana dos pares próximos ao limiar foi concluída sem erros identificados no material revisado, fortalecendo a adequação operacional dessa regra, mas sem garantir identidade única nem excluir duplicidades abaixo do limiar. A rotina \path{scripts/run_groupid_sensitivity.py} --- implementada e pré-declarada --- reconstrói grupos em 0,995, 0,997, 0,999 e 0,9995 e recalcula as métricas; sua grade confirmatória não foi executada integralmente. Os limiares de relevância atuais permanecem heurísticos; distribuições contínuas sustentam a inferência principal.

\section{Validade de conclusão e reprodutibilidade}

Em consonância com boas práticas de pesquisa computacional reprodutível (Sandve et al., 2013), a edição definitiva anterior do código e da documentação está congelada na tag limpa \texttt{paper-v1.1-final}, commit \texttt{6b81953}. A proveniência da corrida científica anterior continua registrada como \texttt{worktree\_dirty=true} e não é reescrita retroativamente. O SHA-256 do modelo de embedding \path{w600k_r50.onnx} é \texttt{4c06341c33c2ca1f86781dab0e829f88ad5b64be9fba56e56bc9ebdefc619e43}; ele foi calculado pelo script \path{scripts/register_model_hash.py}, registrado em \path{artifacts/model_hashes.json} e não é atribuído retroativamente à corrida FairFace original. O subcomando \texttt{cv-fit}, implementado em \path{face_profile_ml/cli.py}, preserva OOF por observação, hash do CSV e ICs bootstrap; como os outputs preservados da corrida principal anterior contêm apenas métricas agregadas, ICs bootstrap não são reportados para essa corrida. A replicação FairFace confirmatória e as grades de ablação, sensibilidade e multiverse permanecem pendentes. Ambiente congelado, hashes de pesos, manifesto completo, preservação de predições OOF e depósito estruturado com DOI devem integrar qualquer nova corrida destinada a ampliar ou replicar os resultados.
