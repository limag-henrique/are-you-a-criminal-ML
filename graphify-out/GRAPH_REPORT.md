...
## Corpus Check
- 187 files · ~870,536 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1521 nodes · 3217 edges · 108 communities (74 shown, 34 thin omitted)
- Extraction: 98% EXTRACTED · 2% INFERRED · 0% AMBIGUOUS · INFERRED: 73 edges (avg confidence: 0.51)
...
- stability.py
- demographic_composition/reporting.py
...
1. `ArtifactBundle` - 43 edges
2. `GallerySimilarityScorer` - 39 edges
3. `FitSpec` - 38 edges
4. `write_csv()` - 38 edges
...
### Community 0 - "cli.py"
Cohesion: 0.17
Nodes (27): ArgumentParser, Path, add_feature_args(), build_parser(), cmd_audit_fairness(), cmd_calibrate(), cmd_cv_fit(), cmd_demo() (+19 more)

### Community 1 - "GallerySimilarityScorer"
Cohesion: 0.06
Nodes (45): BaseHTTPRequestHandler, select_available_providers(), l2_normalize(), ndarray, AppHandler, _clamp(), compose_white_background(), encode_preview_jpeg() (+37 more)
...
Cohesion: 0.06
Nodes (68): _aligned(), compare_scenarios_on_intersection(), cross_fitted_scores(), fit_scenario_run(), _jaccard(), _model(), pairwise_seed_stability(), DataFrame (+60 more)
...
Cohesion: 0.10
Nodes (31): decompose_variance(), _design(), DataFrame, ndarray, Fixed-effect variance decomposition for replicated experiment summaries., Estimate each fixed factor's marginal contribution via reduced models., _sse(), VarianceComponent (+23 more)
...
Cohesion: 0.15
Nodes (15): AnalysisSpec, canonical_json(), Any, Immutable, canonical specifications for reproducible experiments., Serialize a JSON-compatible value deterministically., Return a prefixed, short SHA-256 identity for a canonical value., Factors that determine rule-specific analysis of a reusable fit., _require_nonempty() (+7 more)
...
Cohesion: 0.12
Nodes (26): DataFrame, ndarray, Series, Aggregate validation for probable-duplicate group IDs., Return unlinkable pair hashes near the grouping threshold., safe_threshold_review_sample(), summarize_probable_duplicate_groups(), _folders() (+18 more)
...
Cohesion: 0.07
Nodes (44): ArtifactBundle, BundleValidation, CompletionResult, Any, DataFrame, Path, Atomic, integrity-checked artifact bundles for experiment outputs., Append an explicit, hashed failure classification to ``failures.csv``. (+36 more)
...
### Community 98 - "demographic_composition/reporting.py"
Cohesion: 0.22
Nodes (18): Figure, classify_relevance(), _delta(), _fmt(), _integer_pt(), _markdown_table(), DataFrame, Path (+10 more)
...
### Community 103 - "ArcFaceEmbedder"
Cohesion: 0.27
Nodes (9): ArcFaceEmbedder, _bbox_area(), FaceEmbedding, ndarray, Path, Read an image from disk while preserving Windows Unicode paths and EXIF…, InsightFace ArcFace extractor. The pretrained neural model is used only for…, read_bgr_image() (+1 more)
...
### Community 106 - "read_manifest"
Cohesion: 0.38
Nodes (6): _normalize_quality(), DataFrame, Path, Series, read_manifest(), split_mask()
...
- **Why does `ArcFaceEmbedder` connect `ArcFaceEmbedder` to `cli.py`, `GallerySimilarityScorer`, `run_experiment.py`?**
  _High betweenness centrality (0.070) - this node is a cross-community bridge._
- **Why does `run_grouped_cluster_cv()` connect `run_grouped_cluster_cv` to `cli.py`, `FitSpec`, `build_scenarios`?**
  _High betweenness centrality (0.043) - this node is a cross-community bridge._
- **Why does `write_csv()` connect `write_csv` to `stability.py`, `controls.py`, `run_experiment.py`, `cross_fitting.py`, `predictive_models`, `choose_target_cluster`, `run_all.py`, `source_enrichment`, `run_second_phase.py`, `deduplication.py`, `reconcile`, `clustering_stability.py`?**
